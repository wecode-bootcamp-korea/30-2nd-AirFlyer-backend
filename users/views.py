    ...
    def get(self,request):
        try:
            ...
            for reservation in user_reservations:
                ...
                representative_passenger_information = passenger_informations.first()
                departure_datetime = str(representative_passenger_information.flight_schedule.departure_datetime).split(':')
                arrival_datetime   = str(representative_passenger_information.flight_schedule.arrival_datetime).split(':')

                reservation_information = {
                    'reservation_id' : reservation.id,
                    'seat'           : {
                            'seat_type'   : representative_passenger_information.seat.type,
                            'seat_number' : passenger_informations.count()
                        },
                    'total_price'    : int(passenger_informations.aggregate(total_price=Sum("price"))["total_price"]),
                    'flight_schedule' : {
                            'departure_datetime'    : departure_datetime[0]+':'+departure_datetime[1],
                            'arrival_datetime'      : arrival_datetime[0]+':'+arrival_datetime[1],
                            'duration'              : representative_passenger_information.flight_schedule.duration,
                            'departure_planet_name' : representative_passenger_information.flight_schedule.departure_planet.name,
                            'departure_planet_code' : representative_passenger_information.flight_schedule.departure_planet.code,
                            'arrival_planet_name'   : representative_passenger_information.flight_schedule.arrival_planet.name,
                            'arrival_planet_code'   : representative_passenger_information.flight_schedule.arrival_planet.code,
                        }
                }
